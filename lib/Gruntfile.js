...
module.exports = function(grunt) {
    grunt.initConfig({
        ...
        mxmlc: {
            options: {
                rawConfig: '+optimize=true'
            },
            compile: {
                files: {
                    '<%= dirs.dist %>/swf/muplayer_mp3.swf': '<%= dirs.src %>/as/MP3Core.as'
                }
            }
        },
        ...
        shell: {
            doxx: {
                ...
                command: [
                    'doxx -R <%- dirs.root %>/README.md -t "muplayer 『音频播放内核』" -s <%- dirs.tmp %>/js -T <%- dirs.doc %> --template <%- dirs.jade %>/doc.jade',
                    'mv <%- dirs.doc %>/player.js.html <%- dirs.doc %>/api.html',
                    'cp <%- dirs.src %>/demo.html <%- dirs.doc %>',
                    'mv <%- dirs.dist %>/img/favicon.ico <%- dirs.doc %>'
                ].join('&&')
            }
        },
    ...
    grunt.registerTask('build', ['copy:tmpjs', 'coffee', 'clean:dist', 'webapp-js', 'pc-js',
        'copy-to-dist', 'compass', 'mxmlc', 'concat', 'uglify', 'clean:tmp']);
